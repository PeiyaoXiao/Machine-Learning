...
def preprocess():
    ...
    #Your code here
    #-----Train Data------
    temp_train_stack = np.zeros((0,784))  
    temp_train_labels_stack = np.zeros((0,1),dtype=np.int64)
    for i in range(10): 
    ...
    # Feature Extraction
    res_common_flag = np.all(temp_train_stack == temp_train_stack[0,:],axis=0)
    
    delete_cols = np.where(res_common_flag == True)
    
    temp_train_stack = np.delete(temp_train_stack,delete_cols[0],axis=1)
    ...
    validation_data = validation_data / 255 # Normalization
...
def nnObjFunction(params, *args):
    ...
    for n in range(training_data.shape[0]):
        ...
        z_hidden_output =  np.dot(w1 , training_data[n][:].transpose())                   
        ...
        y = np.zeros(n_class,dtype=np.float64)
        y[training_label[n][0]] = 1.0
        
        neg_log_likelihood_error += -((np.dot(y , np.log(o_output).transpose())) + (np.dot((1.0-y),np.log(1.0-o_output).transpose())))
            
        # Partial derivative of the objective  function wrt weight from input to hidden layer
        
        grad_w1_temp = np.dot(w2[:,0:n_hidden].transpose(),(o_output-y))
        
        grad_w1 += np.dot(((1-z_hidden_output[0:n_hidden]) * z_hidden_output[0:n_hidden] * grad_w1_temp).reshape(n_hidden,1) , training_data[n][:].reshape(n_input+1,1).transpose())
        
        #grad_w1 += np.dot((1-z_hidden_output[0:n_hidden] * z_hidden_output[0:n_hidden] * grad_w1_temp).reshape(3,1) , training_data[n][:].reshape(n_input+1,1).transpose())

        # Partial derivative of the objective  function wrt weight from hidden to output layer
        grad_w2 +=np.dot( ( o_output - y ).reshape(n_class,1) , z_hidden_output.reshape(n_hidden+1,1).transpose() )
    ...
    obj_val = obj_val + ((lambdaval/(2.0*training_data.shape[0]))*reg_term)   
...
training_data = training_data = np.array([np.linspace(0,1,num=5),np.linspace(1,0,num=5)])
training_label = np.array([[0],[1]])
params = np.linspace(-5,5, num=26)

res =nnObjFunction(params ,  n_input, n_hidden, n_class, training_data, training_label, lambdaval)
