...
def nnObjFunction(params, *args):
    ...
    neg_log_likelihood_error = 0
    
    for n in range(training_data.shape[0]):
        #Feedforward Propagation
        #Input to Hidden Layer
        z_hidden_output = np.zeros(n_hidden+1 , dtype=np.float64);
            
        for j in range(n_hidden):
            for i in range(n_input+1):
                z_hidden_output[j] +=  w1[j][i] * training_data[n][i]                   
            
        z_hidden_output = sigmoid(z_hidden_output)
        z_hidden_output[n_hidden] = 1.0 # hidden layer bias node
        
        #Hidden to Output layer
        o_output = np.zeros(n_class , dtype=np.float64);
            
        for l in range(n_class):
            for j in range(n_hidden+1):
                o_output[l] +=  w2[l][j] * z_hidden_output[j]                 
        ...
        for l in range(n_class):
            neg_log_likelihood_error += -((y[l] * np.log(o_output[l])) + ((1.0-y[l])*np.log(1.0-o_output[l])))
            
        # Partial derivative of the objective  function wrt weight from input to hidden layer
        
        for j in range(n_hidden):
            for p in range(n_input+1):
                grad_w1_temp = 0.0
                for k in range(n_class):
                    grad_w1_temp += w2[k][j] * ( o_output[k] - y[k] )
                print grad_w1_temp                                     
                grad_w1[j][p] += (1-z_hidden_output[j])*z_hidden_output[j]*grad_w1_temp*training_data[n][p]
               
        # Partial derivative of the objective  function wrt weight from hidden to output layer
     
        for l in range(n_class):
            for j in range(n_hidden+1):
                grad_w2[l][j] += z_hidden_output[j] * ( o_output[l] - y[l] )
    ...
    for j in range(n_hidden):
        for p in range(n_input+1):
            reg_term = reg_term +  w1[j][p]**2
              
    for l in range(n_class):
        for j in range(n_hidden+1):
            reg_term = reg_term +  w2[l][j]**2
...
def nnPredict(w1,w2,data):
    ...
    for n in range(data.shape[0]):    
        for j in range(n_hidden):
            for i in range(n_input+1):
                z_hidden_output[j] +=  w1[j][i] * data[n][i]                   
            
        z_hidden_output = sigmoid(z_hidden_output)
        z_hidden_output[n_hidden] = 1 # hidden layer bias node
